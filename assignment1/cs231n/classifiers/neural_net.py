...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        hidden_layer = (hidden_layer > 0) * hidden_layer  # relu
        ...
        # classifier loss.                                                          #
        #############################################################################
        true_label_scores = scores[range(N), y].reshape(N, 1)
        exp_sum = np.sum(np.exp(scores), axis=1).reshape(N, 1)
        loss = np.sum(np.log(exp_sum) - true_label_scores) / N
        ...
        temp /= N
        dW2 = np.dot(hidden_layer.T, temp)
        dW2 += reg * W2
        grads['W2'] = dW2
        grads['b2'] = np.sum(temp, axis=0)

        temp2 = np.dot(temp, W2.T)
        temp2[hidden_layer <= 0] = 0
        grads['W1'] = np.dot(X.T, temp2) + reg * W1
